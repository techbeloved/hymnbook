...
class FirebaseRepo @Inject constructor(
    ...
    @Named("FS_COLLECTION") private val collection: String
) : OnlineRepo {
    ...
    override fun hymnIds(@SortBy orderBy: Int): Observable<List<Int>> {
        ...
        return create { emitter ->
            ...
            val successRegistration = hymnCollectionRef.addSnapshotListener(
                executor,
                EventListener { querySnapshot, firestoreException ->
                    if (firestoreException == null) {
                        ...
                        emitter.tryOnError(
                            Throwable(
                                "Error getting songs from firestore!",
                                firestoreException
                            )
                        )
                    }
    ...
    override fun getAllHymns(): Observable<List<OnlineHymn>> {
        return create { emitter ->
            ...
            hymnCollectionRef.get().addOnCompleteListener(
                executor
            ) { querySnapshot ->
                if (querySnapshot.isSuccessful) {
                    val hymns = querySnapshot.result.documents.map { documentSnapshot ->
                        OnlineHymn(
                            id = (documentSnapshot.get("num") as Long).toInt(),
    ...
    override fun getHymnById(id: Int): Observable<OnlineHymn> {
        return create { emitter ->
            ...
            val successRegistration = hymnDocumentRef.addSnapshotListener(
                executor,
                EventListener { documentSnapshot, firestoreException ->
                    if (firestoreException == null && documentSnapshot != null) {
                        ...
                        emitter.tryOnError(
                            Throwable(
                                "Error getting song from firestore!",
                                firestoreException
                            )
                        )
                    }
