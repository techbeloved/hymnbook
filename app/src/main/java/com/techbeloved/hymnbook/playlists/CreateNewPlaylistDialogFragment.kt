...
class CreateNewPlaylistDialogFragment : BottomSheetDialogFragment() {
    ...
    private var _binding: DialogCreateNewPlaylistBinding? = null
    private val binding: DialogCreateNewPlaylistBinding get() = _binding!!
    override fun onCreateView(
        ...
        _binding =
            DataBindingUtil.inflate(inflater, R.layout.dialog_create_new_playlist, container, false)
        ...
        if (arguments != null) {
            val playlistUpdateEvent =
                requireArguments().getParcelable<PlaylistEvent.Update>(EXTRA_PLAYLIST_UPDATE_EVENT)
    ...
    private fun showFavoriteSavedSnackbar(saveStatus: ManagePlaylistViewModel.SaveStatus) {
        when (saveStatus) {
            ManagePlaylistViewModel.SaveStatus.Saved -> {
                Toast.makeText(
                    requireContext(),
                    R.string.success_creating_playlist,
                    Toast.LENGTH_SHORT
                ).show()
            }
            is ManagePlaylistViewModel.SaveStatus.SaveFailed -> {
                if (saveStatus.error is SQLiteConstraintException) {
                    Snackbar.make(
                        ...
                        R.string.error_playlist_already_exist,
